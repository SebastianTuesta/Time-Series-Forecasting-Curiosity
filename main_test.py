...
regr1 = SVR(kernel="rbf", C=100)
regr1.fit(X_train, y_train)
y_predict = regr1.predict(X_test)
...
ensemble = BlendEnsemble(scorer=f1, random_state=rng3)
...
# Max voting
rng3 = np.random.RandomState(0)
ensemble = VotingRegressor(estimators=[("model1", SVR(kernel="rbf", C=100)), ("model2", SVR(kernel="poly"))])
ensemble.fit(X_train, y_train)
y_predict = ensemble.predict(X_test)
